  ...
  <div class="order-list">
    ...
    <el-card shadow="never" class="search-card">
      <el-form :model="query" inline size="default">
        ...
        <el-form-item label="订单类型">
          <el-select v-model="query.order_type" placeholder="全部" clearable style="width: 120px">
    ...
    <el-card shadow="never">
      <el-table :data="tableData" v-loading="loading" stripe border style="width: 100%">
        ...
        <el-table-column prop="payment_method_or_platform" label="支付方式/平台" width="130" />
        <el-table-column label="订单类型" width="90" align="center">
          <template #default="{ row }">
            <el-tag :type="row.order_type === 'dine_in' ? '' : 'warning'" size="small">
              {{ row.order_type === 'dine_in' ? '堂食' : '外卖' }}
            </el-tag>
          </template>
        </el-table-column>
      ...
      <template v-if="detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="订单类型">
            <el-tag :type="detail.order_type === 'dine_in' ? '' : 'warning'" size="small">
              {{ detail.order_type === 'dine_in' ? '堂食' : '外卖' }}
            </el-tag>
...
function handleReset() {
  query.store_id = ''; query.order_type = ''; query.date_from = ''; query.date_to = ''
